...
//	private IsNotChangedValidator validator = new IsNotChangedValidator();
	...
	/*
	 * grid.setCanEdit(false); 모드에선 그리드의 체크박스값이 설정되지 않는다.
	 * 이를 해결하기 위해 grid.setCanEdit(true);로 두고 변경가능한 핸들러를 
	 * 아래와 같이 등록해두고 readOnly 기능을 구현함. 
	 */
...
//		grid.setCanFreezeFields(true);
		...
		//record double click event handler 정의-------------------------------------------------------
...
//				btnSave.enable();
		...
		/*
		 * Selection state 보정.
		 * grid.setGroupByField(..); 가 지정되어 있을 경우 screen에 새로 그려질때 
		 * selection state 및 grouping 정보가 초기화 현상이 발생함.
		 * selection state는 보정처리함  
		 */		
...
//		grid.getRecord(row).setAttribute("print", 	!flag);
	...
	private void correctSelectionRefGroupTypeItem() {
		if(!aclTypeForm.getField("groupType").getCanEdit()) { //Add New 상태가 아닌경우
